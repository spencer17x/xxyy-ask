...
const PRIVATE_CREDENTIAL_PATTERNS = [
  /((?:私钥|助记词|恢复词|密钥)\s*(?:是|为|:|：)?\s*)((?:0x)?[a-fA-F0-9]{64}\b|(?:[a-z]{3,}\s+){11,23}[a-z]{3,}|\[(?:sensitive_credential|REDACTED_PRIVATE_CREDENTIAL)\])/giu,
  /((?:private\s+key|seed\s+phrase|mnemonic|secret\s+recovery\s+phrase)\s*(?:is|:|：)?\s*)((?:0x)?[a-fA-F0-9]{64}\b|(?:[a-z]{3,}\s+){11,23}[a-z]{3,}|\[(?:sensitive_credential|REDACTED_PRIVATE_CREDENTIAL)\])/giu,
