...
第一期不做用户账户查询、交易记录查询或投资建议。交易哈希会进入专用夹子检测路径；默认没有真实数据源时返回“暂未启用”，`TX_ANALYSIS_PROVIDER=browser` 会用本地 Chrome 查询公开交易浏览器和 XXYY 原池子页，当前支持 Solana，并已接入 Base、Ethereum、BSC 浏览器取证初版。
...
- [x] 单笔交易哈希识别、图片附件、Solana 浏览器查询、Base/Ethereum/BSC 取证初版和交易分析报告复查。
- [x] 多链真实样本验收：Solana、Base、Ethereum、BSC，以及裸 EVM 自动识别 Base/Ethereum/BSC。
- [ ] 多轮对话、自动回答总调度、回答质量策略、知识缺口闭环和多渠道接入。
...
apps/
  ...
  cli/        rag:ingest、rag:sync:x、rag:sync:telegram、rag:publish:knowledge、rag:gate:knowledge、rag:migrate、rag:ask、rag:evaluate、rag:feedback 命令
  web/        静态聊天页面
packages/
  shared/     共享类型
  knowledge/  文档加载、chunk、tokenize、embedding provider
  knowledge-ops/  Telegram 授权采集、客服消息脱敏、候选知识挖掘和 Postgres/内存待审队列
...
该命令需要配置 `TELEGRAM_BOT_TOKEN`、逗号分隔的 `TELEGRAM_ALLOWED_CHAT_IDS`，以及可选的 `TELEGRAM_SUPPORT_USER_IDS` 和 `TELEGRAM_UPDATES_LIMIT`。它会使用 Telegram Bot `getUpdates` 增量读取授权 chat，把消息写入 Raw Source Store，生成 `needs_review` 候选知识，并记录下一次拉取 offset；不会发布候选知识，也不会写入正式 RAG 知识库。
...
请求需要带 `Authorization: Bearer <API_OPS_TOKEN>` 或 `x-ops-token: <API_OPS_TOKEN>`。响应会聚合 `/health/deep`、知识库 stats、反馈 stats、工具审计 token 与成本估算、候选知识队列数量、最近 eval 失败、最近自动回答质量缺口摘要和机器可读 `alerts`，用于生产监控、后台页或告警系统；不要把 token 暴露到公开前端。
