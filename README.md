...
第一期不做用户账户查询、交易记录查询或投资建议。交易哈希会进入专用夹子检测路径；默认没有真实数据源时返回“暂未启用”，`TX_ANALYSIS_PROVIDER=mock` 只用于 fixture 演示，`TX_ANALYSIS_PROVIDER=browser` 会用本地 Chrome 查询 Solscan 和 XXYY Discover，当前先支持 Solana。
...
- [x] 交易哈希识别、图片附件、夹子检测 fixture 演示截图和 Solana 浏览器查询骨架。
- [ ] 多轮对话、更多链真实夹子检测、稳定交易窗口筛选、工单创建、人工接管和多渠道接入。
...
本地真实 Solana 交易哈希分析可以设置 `TX_ANALYSIS_PROVIDER=browser`。该模式会启动本机 Chrome，访问 Solscan 和 XXYY Discover，不使用 RPC 或第三方 API key。Solscan 可能触发 Cloudflare 安全验证，保持 `TX_ANALYSIS_BROWSER_HEADLESS=false` 并在弹出的 Chrome 里完成验证后重试即可。截图默认写入 `docs/product-features/assets` 并通过 `/assets/...` 返回。
