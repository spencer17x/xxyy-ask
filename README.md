...
- 暂不暴露 MCP server，也不保留或注册本地 project skills；仓库中的 Capability Plane、EVM 交易/执行/MEV 离线核心、三个只读 RPC adapter、离线组合评测 harness、reviewed replay / production readiness 控制面及其 Postgres control backend 都尚未接入客服运行面。
...
packages/
  ...
  evm-chain-analysis-readiness/  未接线、离线的 reviewed replay 治理与生产就绪控制面
  evm-chain-analysis-control-store/  未接线的 Postgres 治理、审计、预算与 circuit 协调 backend
docs/
