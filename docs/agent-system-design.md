...
- 交易分析：单笔交易哈希识别、browser provider、截图、报告、报告复查。
...
- 交易分析只能使用正式接入的数据源；未配置 provider 时必须返回暂未启用，不生成演示结论。
...
- Tx analysis tool 测试：not configured、browser success、unsupported chain、invalid reference。
...
- 每次工具调用都有结构化审计日志。
- 边界问题不会调用越界工具。
- 后续新增多轮会话、自动回答质量队列、多渠道入口或新知识来源时，只需要新增 connector、工具和 policy，不需要重写核心聊天编排。
