...
如果沿用当前 Markdown loader，第一版可以在发布后触发全量 `rag:ingest` 或 `pnpm sync -- --full`。长期更好的做法是为 reviewed support knowledge 增加 source-aware incremental sync，像 X 增量同步一样只 embedding 新增或变更 chunk。
