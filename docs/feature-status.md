...
- [x] 交易哈希分析路线：聊天入口和 `POST /api/tx-analysis` 都可以接收公开交易哈希或受支持 explorer 链接。`TX_ANALYSIS_PROVIDER=none` 时明确返回暂未启用，`mock` 用于 fixture 演示，`browser` 使用本机 Chrome 查询公开交易浏览器和 XXYY 原池子页。
