...
- [ ] 公开交易分析入口仍暂停：EVM transaction/execution/MEV cores、三个 RPC adapter 与 composition/evaluation harness 都没有 app 配置、Capability 注册或 Agent bridge；聊天中交易、Explorer、链上取证和 MEV 问题继续进入边界/澄清回复。
...
- [ ] 链上能力下一阶段：建立经审批、双人复核且去隐私的公开主网 replay corpus，补足 chain/router/protocol/反例覆盖；完成真实 provider 的跨实例预算、持久审计、告警和 SLO，并实际通过 internal-readiness gate。完成内部授权、Capability adapter 和运行面安全审查前不接入运行面。
