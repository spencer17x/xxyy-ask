...
- [x] MCP 工具：`pnpm run product:mcp:dev` 暴露 `search_product_docs` 和 `answer_product_question`；`pnpm run tx:mcp:dev` 暴露 `analyze_transaction`；`pnpm tx:mcp:smoke` 可跑交易分析 MCP 样本。
- [x] 本地开发命令：启动入口统一为 `pnpm run app:dev`、`pnpm run api:dev`、`pnpm run web:dev`、`pnpm run telegram:dev`、`pnpm run product:mcp:dev` 和 `pnpm run tx:mcp:dev`；知识库更新通过 `app:dev` 的 `--sync`、`--full-sync` 或 `--ingest` 参数显式触发。
