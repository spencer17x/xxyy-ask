...
| XXYY 客服群知识  | `admin_verified` | Telegram 消息证据       | 严格自动治理 |
...
- 客服群原始聊天不能直接入库。Telegram 实时管理员回复或 Desktop JSON 只能先生成脱敏候选，再由版本化严格策略自动批准或拒绝；批准项仍须经过检索门禁、Golden QA、embedding 和事务发布才能成为 `admin_verified`。
