...
4. **持续演进知识库**：官网、官方 X 和客服群知识通过自动清洗、冲突检查、版本化严格决策和隔离发布门禁进入统一知识库。
...
- 未经可信作者验证、严格自动决策和发布门禁的群聊内容不能影响生产回答。
...
| 客服人员          | 快速复用统一、可信的产品答案 | 查看引用、复制回复；群内自然答疑由 Bot 自动采集      |
| 知识管理员        | 观察知识质量和处理系统异常   | 查看自动原因、冲突、发布状态、告警和紧急恢复         |
...
- 未经确定性严格策略批准和隔离发布门禁，不发布 Telegram 群聊知识；模型不能自行批准。
- 不能验证管理员身份、消息时间或上下文时，候选知识必须自动拒绝，不能等待人工把不完整证据升级为可信事实。
...
| FR-306 | P0     | 当前管理员列表不能证明历史消息发送时的角色；系统必须保存角色有效期，无法证明时自动拒绝。            |
| FR-307 | P0     | 匿名管理员消息无法可靠归因时必须自动拒绝。                                                          |
| FR-308 | P0     | Knowledge Curator 能够重建对话线程、脱敏、分类、规范化、去重、识别冲突并生成候选质量分。            |
| FR-309 | P0     | Curator 只能提取候选；独立版本化确定性策略负责自动批准或拒绝，模型不得决定发布。                    |
| FR-310 | P0     | 发布前必须经过作者时效、质量、风险、重复/冲突、边界、检索和回归门禁。                               |
| FR-311 | P1     | 管理后台支持候选上下文、自动原因、冲突对比、发布状态、告警、审计和紧急恢复。                        |
| FR-312 | P1     | 支持知识版本、替代关系、回滚和发布状态追踪。                                                        |
| FR-313 | P0     | Curator 默认自动处理规则未覆盖的复杂线程，并通过单批预算、逐线程失败隔离和脱敏统计限制模型风险。    |
...
flowchart LR
  ...
  Compare --> Pending["短暂 Pending Candidate"]
  Pending --> Policy["版本化严格自动决策"]
  Policy -->|"不满足"| Rejected["Rejected + 原因码"]
  Policy -->|"满足"| Queue["Approved + PublicationJob"]
  Queue --> Gates["边界 / 检索 / 回归门禁"]
...
当前领域实现为保持现有 `packages/*` workspace 边界，分别落在 `packages/transaction-analysis-core`、`packages/evm-data-adapter`、`packages/evm-execution-enrichment-core`、`packages/evm-execution-data-adapter`、`packages/evm-price-impact-sandwich-core`、`packages/evm-mev-observation-data-adapter`、`packages/evm-chain-analysis-harness`、`packages/evm-chain-analysis-readiness`、`packages/evm-chain-analysis-control-store` 和 `packages/evm-chain-analysis-data-plane`：transaction core 计算 normalized transaction/receipt 基础事实；标准 data adapter 实现四方法 JSON-RPC、chain 验证、资源限制、无损归一化与 provider 冲突保留；enrichment core 离线处理有界 call trace、internal transfer、Solidity revert 和带显式 pool metadata 的 Uniswap V2/V3 swap；execution data adapter 使用独立 allowlist 获取 callTracer，并用 pool/factory code、排序 token、V3 fee 和 factory 反查验证 metadata；price-impact/Sandwich core 用有界同区块 observation、pre/post state、counterfactual quote 和 actor token delta 输出四态 verdict；MEV observation adapter 通过 canonical block/order、V2 Sync 或 V3 单 active-range 状态重放、直接 actor token delta 和多 provider fingerprint 生成该 core 的完整输入；chain analysis harness 负责离线阶段组合、跨阶段 fail-closed 锚定、未来 capability projection、synthetic/reviewed corpus 分层和量化质量门禁；readiness 控制面负责来源/保留 approval、分层 sampling policy、确定性 quota plan/manifest/coverage、target-agnostic manifest/candidate handoff、reviewed candidate 单 owner 复核、修订/保留/墓碑、确定性 corpus export，以及 provider budget/audit/alert/circuit/SLO/security evidence 和综合就绪判定；control store 用注入的 Postgres client 提供不可变 sampling/handoff/治理 artifact、sampling/retention/review lease、原子 candidate handoff 与单槽 owner review work queue、哈希链审计、共享 budget、provider request event 和 circuit CAS；data plane 从 opaque secret 构建双 provider adapter，并执行共享控制、cache、持久审计、metrics/alert 与 worker handler。当前仍没有真实来源/法务审批、真实 owner identity、生产 endpoint/credential、reviewed 主网 corpus、已部署 backend/worker、MCP transport、Capability bridge 或通用链上 Skill 接线。领域包增多后再统一迁移到 `packages/skills/*`，不提前重构 monorepo。
...
| `admin_verified` | 官网未覆盖的通用客服知识       | 可信作者 + 严格自动策略 + 发布门禁 |
...
3. 如果只有当前角色而无法证明历史角色，则标记 `historical_role_unverified` 并自动拒绝。
4. 匿名管理员或发送主体为群/频道时自动拒绝。
5. 不允许通过写作风格、频率、昵称相似度推断管理员。

因此，`rag:knowledge:import:telegram` 的 `--admin-id` 可以在可信名册或实时 API 能可靠解析时省略；实时 Bot 默认调用 Telegram API。否则命令失败关闭或自动拒绝候选，不能默认接受所有高频回复者。
...
9. 生成候选质量分、风险标签和确定性决策输入。
10. 保存为短暂 `pending`，由独立 `knowledge-automation-v1` 验证来源、作者时效、最低质量、风险、重复/冲突、Agent lineage 和 prompt injection。
11. 满足全部条件时自动批准并创建唯一 `PublicationJob`；否则自动拒绝并保存原因码。

运行策略默认是 `auto`：模型可用时只处理确定性路径未覆盖的复杂线程，不可用时退化为确定性结果；`deterministic` 禁止模型调用，`required` 在模型、调用或预算失败时整批失败关闭。单批 Agent 调用必须有固定上限，自动模式的失败统计不能包含 Provider 异常原文或群聊消息正文。
...
| 在线 Agent 控制面 | LangGraph JS                                             | 产品包含显式路由、领域子图、并行工具调用、证据状态和暂停恢复；这些是图状态而不是简单的 Agent handoff。知识治理在 Agent 图外使用确定性策略。                     |
...
| 长任务与可靠执行  | Temporal，达到触发条件后引入                             | 适合知识同步、索引重建、长时间链上分析、跨重启有限重试和补偿；普通在线问答不经过 Temporal。                                                                     |
...
- 实现版本化严格自动批准/拒绝、发布入队、对账和三次重试上限。
- 上线最小可观测与紧急恢复后台。

验收：正常流程无需逐条人工操作；不满足严格策略的群聊自动拒绝；每条已发布知识均可追溯和回滚。
...
- 群聊知识只能通过可信作者验证、Curator、版本化严格自动决策和发布门禁进入正式知识库。
