...
| --------------------------- | -------------------------------- | ------------------------------- | ----------------------------------------------- |
| `chain.inspect_transaction` | moderate / external read         | `chain.public`                  | 离线 EVM core 已实现；adapter/capability 未注册 |
...
第一批真正接入的能力应只选公开数据、只读、可离线评测的链上分析，并先限于内部或管理 channel。随后单独实现 Skill/MCP adapter、网络 allowlist、RPC 配额、结果证据与失败降级，再通过显式 bridge 选择是否暴露给 LangGraph；Capability Plane 本身不会让 Planner 自动发现或执行能力。
