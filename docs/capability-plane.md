...
| --------------------------- | -------------------------------- | ------------------------------- | ------------------------------------------------- |
| `chain.inspect_transaction` | moderate / external read         | `chain.public`                  | core + RPC data adapter 已实现；capability 未注册 |
...
第一批真正接入的能力应只选公开数据、只读、可离线评测的链上分析，并先限于内部或管理 channel。底层 RPC allowlist、资源限制、snapshot provenance 和失败降级已经由独立 data adapter 实现，但仍需生产 provider 配额/观测、Skill/Capability adapter、授权 grant、显式 bridge 和端到端评测；Capability Plane 本身不会让 Planner 自动发现或执行能力。
