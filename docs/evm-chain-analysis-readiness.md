...
- companion control store 用独立 sampling planner/worker/submitter/reviewer role、不可变 artifact、`FOR UPDATE SKIP LOCKED`、lease/attempt fencing、原子 candidate handoff、双槽 review work queue 和哈希链审计持久化该流程。

manifest 不是 reviewed replay candidate，更不是 ground truth。v0.14b2a2 已定义显式 handoff：从持久化 manifest 继承 source/retention lineage，重新扫描 normalized replay payload，闭合 chain/transaction/block、protocol/route/data-state 和时间锚点，再在一个 Postgres 事务中创建初始 candidate、retention job 与审计 link。v0.14b2a3 同事务创建两个确定性 review slot，并要求 reviewer 以 `jobId + attemptCount` fenced lease 提交结果。sampling target 与 proposed ground truth 不一致只记录 `deviated`，不会拒绝样本。真实 worker 和 reviewer 仍未部署，candidate 仍须经过真实双人 review 和 promotion。完整契约见 [Mainnet Sampling Plan & Evidence Intake Control Plane](evm-chain-analysis-sampling.md)、[Sampling Manifest → Reviewed Replay Candidate Handoff](evm-chain-analysis-sampling-handoff.md) 与 [Independent Review Work Queue](evm-chain-analysis-review-work-queue.md)。
...
sampling handoff candidate 还必须先从两个确定性 review slot 中领取一槽。control store 排除 submitter、已有 review 的身份及同 candidate 另一槽的 owner，使用 `FOR UPDATE SKIP LOCKED`、lease expiry 与 attempt generation 支持并发和安全重领。review artifact、job completion 与 audit 原子提交。普通非 handoff candidate 仍使用原有直接 review 契约；完整数据库状态机见 [Independent Review Work Queue](evm-chain-analysis-review-work-queue.md)。
