...
- `@xxyy/evm-chain-analysis-control-store` 保存不可变 artifact，并提供带角色授权、lease、重试、原子 candidate handoff、双槽 review work queue 和审计的 intake 状态机；
...
虚线节点仍未实际执行。manifest 只是采集元数据和 payload hash，不是 ground truth。已实现的 handoff 会重新扫描 replay payload 并创建 `pending_review` candidate，但不会绕过 replay、双人复核、promotion 和 corpus export。
...
- manifest/candidate 锚点、target deviation 保留、handoff 一对一、原子 candidate/retention/two-review-slot/audit 和幂等重试；
...
本阶段还用一次性真实 PostgreSQL 数据库验证了迁移幂等、artifact append-only trigger、quota job、失败/重试/完成、审计链、coverage，以及偏差 handoff 的 candidate/retention/two-review-slot 原子写入和幂等重试；review queue 阶段继续验证了独立领取、完成与 attempt 上限。验证库随后已删除。这仍是合成契约验证，不是生产部署或主网 evidence。

## 下一阶段

后续 v0.14b2b 才能在包外执行真实工作：

1. 由有权人员完成并保管真实来源、法律和保留评审，映射真实 planner/worker/reviewer identity；
2. 部署最小权限 Postgres、sampling/retention/reconciliation worker、secret manager 和 provider；
3. 按 plan 采集公开链 payload，通过已定义 handoff 转为 candidate，再执行 replay 与双人独立复核；
