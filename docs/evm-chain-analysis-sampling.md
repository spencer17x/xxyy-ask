...
- `@xxyy/evm-chain-analysis-control-store` 保存不可变 artifact，并提供带角色授权、lease、重试、原子 candidate handoff 和审计的 intake 状态机；
...
control store 要求 `candidate_submitter` grant，在单个事务中写 candidate、唯一 retention job、不可变 handoff，以及 candidate/handoff 两条 hash-chain audit link；manifest/candidate 都是一对一。相同 handoff 幂等，冲突 handoff 或已绕过 handoff 单独写入的 candidate 会 fail closed。完整设计见 [Sampling Manifest → Reviewed Replay Candidate Handoff](evm-chain-analysis-sampling-handoff.md)。
...
- manifest/candidate 锚点、target deviation 保留、handoff 一对一、原子 candidate/retention/audit 和幂等重试；
...
本阶段还用一次性真实 PostgreSQL 数据库验证了迁移幂等、artifact append-only trigger、quota job、失败/重试/完成、审计链、coverage，以及偏差 handoff 的 candidate/retention 原子写入和幂等重试；验证库随后已删除。这仍是合成契约验证，不是生产部署或主网 evidence。
