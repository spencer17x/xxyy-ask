...
- `@xxyy/evm-chain-analysis-readiness` 定义 content-addressed 来源/保留审批记录、分层采样策略、确定性 quota plan、公开链样本 manifest 和 coverage result；
- `@xxyy/evm-chain-analysis-control-store` 保存不可变 artifact，并提供带角色授权、lease、重试和审计的 intake job 状态机；
...
flowchart LR
  ...
  Manifest --> Candidate["Reviewed replay candidate intake"]
  Candidate --> Review["双人独立复核"]
  Review --> Corpus["Governed reviewed corpus"]

  style Worker stroke-dasharray: 5 5
  style Candidate stroke-dasharray: 5 5
  style Review stroke-dasharray: 5 5
  style Corpus stroke-dasharray: 5 5
```

虚线节点仍未实际执行。manifest 只是采集元数据和 payload hash，不是 ground truth，也不会绕过后续敏感信息扫描、replay、双人复核、promotion 和 corpus export。
...
- 保留截止时间由 `collectedAt + retentionDays` 确定；
...
本阶段还用一次性真实 PostgreSQL 数据库验证了迁移幂等、artifact append-only trigger、3 个 quota job、失败/重试/完成、审计链和 complete coverage；验证库随后已删除。这仍是合成契约验证，不是生产部署或主网 evidence。
...
3. 按 plan 采集公开链 payload，将 manifest 转为 candidate，再执行 replay 与双人独立复核；
