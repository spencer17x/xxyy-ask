...
独立、allowlisted 的 [EVM Execution Data Adapter](evm-execution-data-adapter.md)、[MEV Observation Data Adapter](evm-mev-observation-data-adapter.md) 及交叉 provider replay，离线 [EVM Price Impact / Sandwich Core](evm-price-impact-sandwich.md) 和 [Chain Analysis Harness](evm-chain-analysis-harness.md) 已完成。仍缺 reviewed 主网 corpus、真实 provider、跨实例共享配额/审计/告警、内部授权和运行面安全审查；这些完成前不注册链上 Capability。
