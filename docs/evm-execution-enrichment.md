...
独立、allowlisted 的 [EVM Execution Data Adapter](evm-execution-data-adapter.md)、[MEV Observation Data Adapter](evm-mev-observation-data-adapter.md) 及交叉 provider replay，以及离线 [EVM Price Impact / Sandwich Core](evm-price-impact-sandwich.md) 已完成。仍缺离线组合与人工标注评测、真实 provider、跨实例共享配额/审计/告警、内部授权和运行面安全审查；这些完成前不注册链上 Capability。
