...
独立、allowlisted 的 [EVM Execution Data Adapter](evm-execution-data-adapter.md) 及交叉 provider replay，以及离线 [EVM Price Impact / Sandwich Core](evm-price-impact-sandwich.md) 已完成，但仍缺完整 block/transaction-boundary state 的 MEV observation adapter、真实 provider 和共享 QPS/熔断/缓存/metrics。在生产数据面、内部授权、运行面安全审查和端到端评测完成前，不注册链上 Capability。
