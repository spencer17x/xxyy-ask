...
该包不获取 trace，不调用 RPC、Indexer 或 Explorer，不读取环境变量，不依赖 LLM、LangGraph、`ToolRegistry`、`CapabilityRegistry` 或 MCP。现有 `@xxyy/evm-data-adapter` 仍只读取 transaction、receipt、chain id 和 block，没有提供 trace 或 pool metadata。仓库中也没有 composition root 把这些包连接到 API、CLI、Telegram 或客服 Agent；交易、Explorer、链上取证和 MEV 问题继续返回现有边界回复。
...
flowchart LR
  ...
  Rpc["Trace / Metadata Provider"] -. "未实现" .-> Trace
...
Metadata 是带 provenance 的输入事实，不等于已经链上验证。未来 adapter 必须从 allowlisted provider 获取并交叉验证 pool code/factory/token；在那之前，结果用独立的 `metadata` Evidence kind 保留 source，调用方必须把可信度限制在该来源范围内。
...
v0.1 的 delta 是事件声明的 pool balance delta，不等于独立验证过的 token Transfer 总和、用户实际成交额或价格影响。多 pool 路由也不会自动合并为一笔用户 swap。
...
- debug/trace RPC、archive node、Indexer 或 Explorer trace adapter；
- pool factory/code/token metadata 的链上获取与多 provider 交叉验证；
...
- block 内邻近交易、候选 frontrun/backrun、攻击者聚类和 Sandwich 四态 verdict；
- Capability manifest/adapter、MCP、LangGraph bridge、API/CLI/Telegram 入口；
- 私有账户数据、签名、模拟、交易发送或投资建议。

下一阶段应先实现独立、allowlisted、带配额和缓存的 trace/pool metadata data adapter，并补交叉 provider fixtures。随后才能用 block 交易集合、池状态与资产变化实现离线价格影响和 Sandwich detection core；在内部授权、运行面安全审查和端到端评测完成前，不注册 `chain.inspect_transaction`。
