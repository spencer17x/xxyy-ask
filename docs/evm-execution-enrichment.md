...
独立、allowlisted 的 [EVM Execution Data Adapter](evm-execution-data-adapter.md) 及交叉 provider replay 已完成，但仍未配置生产 provider，也没有共享 QPS/熔断/缓存/metrics。下一阶段可以用 block 交易集合、池状态与资产变化实现离线价格影响和 Sandwich detection core；在生产数据面、内部授权、运行面安全审查和端到端评测完成前，不注册 `chain.inspect_transaction`。
