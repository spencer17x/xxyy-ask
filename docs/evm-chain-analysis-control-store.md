...
- sampling source approval、policy、plan、sample manifest 和 coverage run；
- audit event；
...
approval/policy/plan/manifest/run 是 append-only artifact；job 是协调状态。control store 不采集 payload、不调用 provider，也不把 manifest 自动转换成 reviewed candidate。contract-only tests 不能代表来源或法律评审完成。详细设计见 [Mainnet Sampling Plan & Evidence Intake Control Plane](evm-chain-analysis-sampling.md)。
